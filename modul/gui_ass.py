...
def changeTheme():
    ...
    if themeValue.get() == 1:
        ...
        botChatText, botChatTextBg, userChatTextBg = "white", "#3897d3", "#3897d3"
        chatBgColor = "#12232e"
        colorbar['bg'] = chatBgColor
    else:
        background, textColor, AITaskStatusLblBG, KCS_IMG = "#F6FAFB", "#303E54", "#3897d3", 0
...
def main(text):
    if "project" in text:
        if isContain(text, ['make', 'create']):
            ...
            speak(file_handler.CreateHTMLProject(
    ...
    if isContain(text, ['news']):
        ...
        if isContain(text, ["no", "don't"]):
            ...
            web_scrapping.openWebsite('https://vnexpress.net/thoi-su')
...
def UpdateIMAGE():
    ...
    os.system('python modul/avatar_selection.py')
    u = UserData()
    u.extractData()
    ownerPhoto = u.getUserPhoto()
    userProfileImg = ImageTk.PhotoImage(Image.open(
        "assets/images/avatars/a"+str(ownerPhoto)+".png").resize((120, 120)))
    userPhoto['image'] = userProfileImg
    userIcon = PhotoImage(
        file="assets/images/avatars/ChatIcons/a"+str(ownerPhoto)+".png")
...
def progressbar():
    s = ttk.Style()
    s.theme_use('clam')
    s.configure("color.Horizontal.TProgressbar",
                foreground='#47b5ff', background='#47b5ff')
    progress_bar = ttk.Progressbar(splash_root, style="color.Horizontal.TProgressbar",
...
if __name__ == '__main__':
    # splash
    splash_root = Tk()
    splash_root.configure(bg='#06283D')
    splash_root.overrideredirect(True)
    splash_label = Label(splash_root, text='J.A.R.V.I.S', font=(
        'Monsterrat', 20), bg='#256D85', fg='#DFF6FF')
    splash_label.pack(pady=40)
    splash_percentage_label = Label(
        splash_root, text='0 %', font=('Monsterrat', 8))
    splash_percentage_label.pack(padx=(0, 10))
    ...
    userProfileImg = Image.open(
        "assets/images/avatars/a"+str(ownerPhoto)+".png")
    ...
    chngPh = ImageTk.PhotoImage(Image.open(
        "assets/images/avatars/changephoto2.png").resize((120, 120)))
    ...
    backBtn = Button(settingsFrame, text='   Back   ', bd=0, font=(
        'Arial 12'), fg='white', bg='#3897d3', relief=FLAT, command=lambda: raise_frame(root1))
    clearData = Button(settingsFrame, text='   Clear Data   ', bd=0, font=(
        'Arial 12'), fg='white', bg='#3897d3', relief=FLAT, command=deleteUserData)
    backBtn.place(x=5, y=250)
    clearData.place(x=120, y=250)
    try:
        # pass
